  ...
  constructor(options) {
    ...
    if (options.setupType == Enums.SetupType.InitialAlphabetWithRotors) {
      ...
      for (let r = 0; r < options.toAlphabets.length; r++) {
        encRotors.push(new Rotor(options.fromAlphabet.letters, options.toAlphabets[r].letters))
        decRotors.push(new Rotor(options.fromAlphabet.letters, JSON.parse(JSON.stringify(options.toAlphabets[r].letters))))
      }
      ...
      for (let r = 0; r < options.rotorCount; r++) {
        ...
        decRotors.push(new Rotor(alphabet.letters, JSON.parse(JSON.stringify(shuffled))))
      }
    } else if (options.setupType == Enums.SetupType.InitialAlphabet) {
      // Given a source alphabet only. Randomly create rotors.
      for (let r = 0; r < options.rotorCount; r++) {
        let shuffled = options.alphabet.shuffleNew()
        encRotors.push(new Rotor(options.alphabet.letters, shuffled))
        decRotors.push(new Rotor(options.alphabet.letters, JSON.parse(JSON.stringify(shuffled))))
      }
    } else {
