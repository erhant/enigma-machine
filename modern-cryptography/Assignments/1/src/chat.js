...
function refresh(username) {
  ...
  for (let i = 0; i<history.length; i++) {
    console.log(history[i].username.green+" ("+history[i].timestamp.yellow+"): "+enigma.decrypt(history[i].text))
  }
...
function refreshWithoutDecrypting() {
  ...
  let history = JSON.parse(fs.readFileSync("./chat/history.json", 'utf8'));
  for (let i = 0; i<history.length; i++) {
    console.log(history[i].username.green+" ("+history[i].timestamp.yellow+"): "+history[i].text)
  }
...
async function bombeRoutine(rl, c) {
  ...
  let keywordNo = 1;
  console.clear();
  console.log("Enter some keywords now. To start breaking, just enter without typing anyting!");
  while (true) {
